...
public:
  ...
  MemoryResourceProvider(const std::string& pmem_path, size_t size);

  std::pmr::memory_resource* get(const MemRequirements& req) const;
  MemType getMemoryType(std::pmr::memory_resource* other);
  size_t getAvailableMemorySize(std::pmr::memory_resource* mem_resource);
private:
